...
dyt<-nt
yt<-matrix(0,n,1)
for(t in 2:nn) {yt[t]<-(ph*dyt[t-1]+nt[t])+yt[t-1]}
yt<-yt
}
...
TDist[i,1] <- sum(tA$tadf<crA)
TDist[i,2] <- sum(tB$tadf<crB)
TDist[i,3] <- sum(tC$tadf<crC)
}

sA <- (sum(TDist[,1])*100)/rep
sB <- (sum(TDist[,2])*100)/rep
sC <- (sum(TDist[,3])*100)/rep
...
printsz <- function(SZ){
cat ("\n")
cat ("Size value for Selected Sample Size"                                                , "\n")
cat ("              Nominal Size: 5%"                                                     , "\n")
...
cat ("   ", round(SZ[1],3),"     ",round(SZ[2],3),"     ",round(SZ[3],3)                  , "\n")
cat ("\n")
}
